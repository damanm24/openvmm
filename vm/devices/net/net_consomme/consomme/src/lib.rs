...
struct ConsommePrimary {
    ...
    icmp: icmp::Icmp,
    #[inspect(mut)]
    tcp_listeners: tcp::TcpListeners,
    #[inspect(mut)]
    udp_listeners: udp::UdpListeners,
}
...
/// Opaque transport flow state that can be moved between Consomme shards.
...
const MAX_EGRESS_BYTES: usize = 4 * 1024 * 1024;

/// An owned packet generated by Consomme for delivery to the guest.
...
impl EgressPacket {
    ...
    /// Returns the checksum metadata for the packet.
    pub fn checksum(&self) -> ChecksumState {
        self.checksum
    }
}
...
impl EgressQueue {
    ...
    fn recycle(&mut self, mut packet: EgressPacket) {
        packet.data.clear();
...
/// Provides the async driver used to register Consomme-owned sockets.
pub trait SocketDriver {
    ...
    /// Captures an owned guest-bound packet in unit tests.
    #[cfg(test)]
    fn capture_egress(&mut self, _data: &[u8], _checksum: ChecksumState) {}
}
...
impl Consomme {
    ...
    pub fn new(config: ConsommeConfig, mut params: ConsommeParams) -> Self {
        ...
        let shard_config = ShardConfigSnapshot {
            generation: 0,
    ...
    /// Gets the current runtime parameters.
    pub fn params(&self) -> &ConsommeParams {
        &self.primary.config.params
    }
    ...
    pub fn update_params(&mut self, update: impl FnOnce(&mut ConsommeParams)) {
        ...
        self.shard.config.generation = self.shard.config.generation.wrapping_add(1);
        self.shard.config.params = self.primary.config.params.clone();
    ...
    pub fn access<'a, T: SocketDriver>(&'a mut self, client: &'a mut T) -> Access<'a, T> {
        Access {
            inner: self,
            client,
        }
    ...
    /// Removes the next guest-bound packet from the bounded egress queue.
    pub fn pop_egress(&mut self) -> Option<EgressPacket> {
        self.shard.egress.pop()
    }

    /// Returns a delivered packet buffer to the bounded recycling pool.
    pub fn recycle_egress(&mut self, packet: EgressPacket) {
    ...
    pub fn drain_flows(&mut self) -> FlowState {
        FlowState {
            tcp: self.shard.tcp.drain_flows(),
            udp: self.shard.udp.drain_flows(),
        }
    }

    /// Inserts previously drained TCP and UDP flow state into this shard.
...
impl<T: SocketDriver> Access<'_, T> {
    ...
    #[cfg(test)]
    fn capture_test_egress(&mut self) {
        while let Some(packet) = self.inner.pop_egress() {
            self.client.capture_egress(packet.data(), packet.checksum());
            self.inner.recycle_egress(packet);
        }
    }
